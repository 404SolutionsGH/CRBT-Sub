...
export const signUpController = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (password && accountType && username) {
    ...
    await sendConfirmationMessage(account.authorizationMethod, verfCode, email, phone, username);
...
export const resetAccountController = asyncHandler(async (req: Request, res: Response) => {
  ...
  const newPassword = `${account.username}${verfCodeGenerator()}4563`;
  ...
  await sendAccountResetEmail(account.username, newPassword, account.email);
...
export const sendConfirmationCodeController = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (email) {
    ...
    if (account) {
      await sendConfirmationMessage(account.authorizationMethod, verfCode, email, account.phone, account.username);
    ...
    console.log("Account updated");
    if (account) {
      await sendConfirmationMessage(account.authorizationMethod, verfCode, account.email, account.phone, username);
      authorizationMethod = account.authorizationMethod;
    }
  }
