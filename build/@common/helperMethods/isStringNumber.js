...
const isStringContentNumber = (content, variableName) => {
    ...
    if (alphaReg.test(content)) {
        throw new AppError_1.AppError(`Item ${variableName} must be an integer not a character or alphanumeric characters`, 400);
    }
    else if (decimalReg.test(content)) {
        throw new AppError_1.AppError(`Item ${variableName} must be an integer not decimal`, 400);
    }
};
