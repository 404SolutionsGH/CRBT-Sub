...
export const getAdminAccountInfo = async (adminId: number) => {
  ...
  return { adminType, lastName, firstName, email,role, profile,nextSubPayment, createdAt, subPlanDetails: await findPlanById(planId), rewardPoints: rewardInfo ? rewardInfo.points : 0 };
};
